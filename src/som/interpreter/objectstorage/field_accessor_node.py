...
class _AbstractReadFieldNode(_AbstractFieldAccessorNode):
    ...
    def _specialize(self, obj, _reason, next_read_node):
        ...
        node = _SpecializedReadFieldNode(
            self._field_idx, layout, location, self._depth, next_read_node
        )
...
class _UninitializedReadFieldNode(_AbstractReadFieldNode):
    def read(self, obj):
        ...
        if self._depth < _MAX_CHAIN_LENGTH:
            next_node = _UninitializedReadFieldNode(self._field_idx, self._depth + 1)
        else:
...
class _SpecializedReadFieldNode(_AbstractReadFieldNode):
    ...
    def __init__(self, field_idx, layout, location, depth, next_read_node):
...
class _AbstractWriteFieldNode(_AbstractFieldAccessorNode):
    def _write_and_respecialize(self, obj, value, _reason, next_write_node):
        obj.set_field(self._field_idx, value)
        return self._respecialize(obj, value, next_write_node)

    def _respecialize(self, obj, _value, next_write_node):
        layout = obj.get_object_layout()
        location = layout.get_storage_location(self._field_idx)
        node = _SpecializedWriteFieldNode(
            self._field_idx, layout, location, self._depth, next_write_node
        )
        return self.replace(node)
...
class _UninitializedWriteFieldNode(_AbstractWriteFieldNode):
    def write(self, obj, value):
        if self._depth < _MAX_CHAIN_LENGTH:
            next_node = _UninitializedWriteFieldNode(self._field_idx, self._depth + 1)
        else:
            next_node = _GenericWriteFieldNode(self._field_idx, self._depth + 1)
        self._write_and_respecialize(obj, value, "initialize write node", next_node)
        return value
...
class _SpecializedWriteFieldNode(_AbstractWriteFieldNode):
    ...
    def __init__(self, field_idx, layout, location, depth, next_write_node):
    ...
    def _do_write(self, obj, value):
        try:
            ...
            obj.update_layout_with_initialized_field(self._field_idx, value.__class__)
        except GeneralizeStorageLocationException:
            obj.update_layout_with_generalized_field(self._field_idx)
    ...
    def write(self, obj, value):
        if self._layout is obj.get_object_layout():
            ...
            if self._layout.is_for_same_class(obj.get_class(None)):
                self._write_and_respecialize(
                    obj, value, "update outdated node", self._next
                )
            else:
