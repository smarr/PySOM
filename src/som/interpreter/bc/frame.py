...
from som.interpreter.ast.frame import (
    ...
    _FRAME_INNER_IDX,
    _INNER_ON_STACK_IDX,
    _FRAME_AND_INNER_FIRST_ARG, read_frame,
)
...
def create_frame(
    arg_inner_access, size_frame, size_inner, before_stack_start, prev_frame, num_args
):
    ...
    assert num_args - 1 == len(arg_inner_access)  # num_args without rcvr
    ...
    if size_inner > 0:
        ...
        _set_arguments_with_inner(frame, inner, arg_inner_access, prev_frame, num_args - 1)

        for o in inner:
            from som.vmobjects.abstract_object import AbstractObject
            assert isinstance(o, AbstractObject)
    else:
...
@jit.unroll_safe
def _set_arguments_with_inner(frame, inner, arg_inner_access, prev_frame, num_args_without_rcvr):
    arg_i = num_args_without_rcvr - 1
    frame_i = 2
    ...
    while arg_i >= 0:
        if arg_inner_access[arg_i]:
            inner[inner_i] = get_stack_element(prev_frame, arg_i)
            inner_i += 1
        else:
            frame[frame_i] = _erase_obj(get_stack_element(prev_frame, arg_i))
            frame_i += 1
...
def stack_top(frame):
    stack_pointer = jit.promote(_unerase_sptr(frame[-1]).sp)
...
def stack_set_top(frame, value):
    stack_pointer = _unerase_sptr(frame[-1]).sp
    assert stack_pointer < len(frame) - 1
...
def stack_pop(frame):
    ...
    stack_pointer = jit.promote(sptr.sp)
    assert stack_pointer < len(frame) - 1

    sptr.sp = stack_pointer - 1
...
def stack_push(frame, value):
    ...
    sptr = _unerase_sptr(frame[-1])
    stack_pointer = jit.promote(sptr.sp + 1)
    assert stack_pointer < len(frame) - 1

    assert value is not None
    frame[stack_pointer] = _erase_obj(value)
    sptr.sp = stack_pointer
...
def stack_reset_stack_pointer(frame, before_stack_start):
    ...
    the stack"""
    _unerase_sptr(frame[-1]).sp = before_stack_start
