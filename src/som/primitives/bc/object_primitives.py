from som.interpreter.bc.frame import stack_pop, stack_top, stack_push
...
def _perform_with_arguments(_ivkbl, frame):
    args = stack_pop(frame)
    selector = stack_pop(frame)
    rcvr = stack_top(frame)

    for i in range(0, args.get_number_of_indexable_fields()):
        stack_push(frame, args.get_indexable_field(i))
